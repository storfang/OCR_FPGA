...
parameter FXP_SHIFT = 14 )(
    input wire signed [19:0]  x[LAYER2-1:0],
    input wire signed [19:0] d,
    input wire rst,
    input wire clk,
    input wire mode,
    output reg signed [19:0] g_delta[LAYER2-1:0],
    output reg signed [19:0]  y
    );

    reg signed [19:0] w[LAYER2-1:0] ={-0.5*FXP_SCALE*m, 0.24*FXP_SCALE*m, 0.43*FXP_SCALE*m, 0.16*FXP_SCALE*m, -0.4*FXP_SCALE*m,
    ...
    reg signed [19:0] w_nxt[LAYER2-1:0] ={-0.5*FXP_SCALE*m,0.24*FXP_SCALE*m, 0.43*FXP_SCALE*m, 0.16*FXP_SCALE*m, -0.4*FXP_SCALE*m,
                     0.21*FXP_SCALE*m, 0.3942*FXP_SCALE*m, -0.5*FXP_SCALE*m,-0.2*FXP_SCALE*m, 0.583*FXP_SCALE*m, 0.23*FXP_SCALE*m,
                      -0.1*FXP_SCALE*m, 0.579*FXP_SCALE*m, -0.41*FXP_SCALE*m, 0.468*FXP_SCALE*m, 1*FXP_SCALE*m};
    reg signed [39:0] sum = 0; reg signed [19:0] sum_nxt = 0;
